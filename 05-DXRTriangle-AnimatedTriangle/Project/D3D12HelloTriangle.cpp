...
void D3D12HelloTriangle::OnUpdate()
{
	...
	// #DXR Extra - Refitting
	// Increment the time counter at each frame
	m_time++;
	// update the corresponding instance matrix of the first triangle to animate its position
	m_instances[0].second =
		DirectX::XMMatrixRotationAxis({0.f, 1.f, 0.f}, static_cast<float>(m_time) / 50.0f) *
		DirectX::XMMatrixTranslation(0.f, 0.1f * cosf(m_time / 20.f), 0.f);
	// update the corresponding instance matrix of the second triangle to animate its position
	m_instances[1].second =
		DirectX::XMMatrixRotationAxis({0.f, 1.f, 0.f}, static_cast<float>(m_time) / 10.0f) *
		DirectX::XMMatrixTranslation(.6f, 0.1f * cosf(m_time / 20.f), 0.f);
	// update the corresponding instance matrix of the third triangle to animate its position
	m_instances[2].second =
		DirectX::XMMatrixRotationAxis({0.f, 1.f, 0.f}, static_cast<float>(m_time) / 100.0f) *
		DirectX::XMMatrixTranslation(-.6f, 0.1f * cosf(m_time / 20.f), 0.f);
}
...
//
void D3D12HelloTriangle::UpdateCameraBuffer() const
{
