...
namespace nv_helpers_dx12
{
	...
	//
	//
	inline ID3D12Resource* CreateBuffer(ID3D12Device* m_device, const uint64_t size,
	...
	//
	//
	inline ID3D12DescriptorHeap* CreateDescriptorHeap(ID3D12Device* device, const uint32_t count,
	...
	void GenerateMengerSponge(const int32_t level, const float probability, std::vector<Vertex>& outputVertices,
	                          std::vector<UINT>& outputIndices)
	{
		struct Cube
		{
			Cube(const XMVECTOR& tlf, const float s) : m_topLeftFront(tlf), m_size(s)
			{
			}
			...
			void enqueueQuad(std::vector<Vertex>& vertices, std::vector<UINT>& indices,
			                 const XMVECTOR& bottomLeft4, const XMVECTOR& dx, const XMVECTOR& dy, const bool flip)
			{
				const UINT currentIndex = static_cast<UINT>(vertices.size());
				...
				vertices.push_back({
					{
						bottomLeft.x + dx.m128_f32[0], bottomLeft.y + dx.m128_f32[1],
				...
				vertices.push_back({
					{
						bottomLeft.x + dy.m128_f32[0], bottomLeft.y + dy.m128_f32[1],
				...
				vertices.push_back({
					{
						bottomLeft.x + dx.m128_f32[0] + dy.m128_f32[0],
			...
			void splitProb(std::vector<Cube>& cubes, const float prob)
			{
				float size = m_size / 3.f;
				XMVECTOR topLeftFront = m_topLeftFront;
				for (int x = 0; x < 3; x++)
				{
					...
					for (int y = 0; y < 3; y++)
					{
						...
						for (int z = 0; z < 3; z++)
						{
							const float sample = rand() / static_cast<float>(RAND_MAX);
							if (sample > prob)
								continue;
							topLeftFront.m128_f32[2] = m_topLeftFront.m128_f32[2] + static_cast<float>(z) * size;
							cubes.push_back({topLeftFront, size});
