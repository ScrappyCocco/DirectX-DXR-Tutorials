...
void DirectXUtil::AccelerationStructures::createPrimitive(SampleFramework::ID3D12Device5Ptr pDevice)
{
	...
	//Vertex
	createdPrimitive->vertexCount = static_cast<unsigned int>(shape.vertexData.size());
	createdPrimitive->vertexBuffer = createBuffer(
		pDevice,
	...
	//Index
	createdPrimitive->indexCount = static_cast<unsigned int>(shape.indexData.size());
	createdPrimitive->indexBuffer = createBuffer(
		pDevice,
	...
	SampleFramework::ID3D12Device5Ptr pDevice, SampleFramework::ID3D12GraphicsCommandList4Ptr pCmdList)
{
	...
	ShapeResources* shape = createdPrimitive;
